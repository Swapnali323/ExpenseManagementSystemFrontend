...
this.projectids.push(element.projectCode)
  ...
  {
    if (form.valid) {
      this.claimservice.searchJPAClaim(this.claimid).subscribe(
        res => {
...
{
  if (form.valid) {
    this.claimservice.searchJPAEmployeeClaims(this.employeeid).subscribe(
      res => {
...
/*****************************************first search email of the employee************** */
    console.log("employee info"+employeeId)
    
    this.employeeservice.searchJPAEmployee(employeeId).subscribe(
      res => {
        
        this.emailemployees = res
        this.esize=this.emailemployees.length
        for(let i=0;i<this.emailemployees.length ;i++)
        console.log("employee id sender email" +this.emailemployees[0].mailId)
       /**********************************send email to concerned employee */
        this.approvalservice.sendEmail(this.emailemployees[0].mailId,approval).subscribe(
          res => {
